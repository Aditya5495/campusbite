...
import { ShoppingCart, User, LogOut, UtensilsCrossed, Store } from "lucide-react";
        ...
        <div className="flex items-center gap-4">
          ...
          {user ? (
            <div className="flex items-center gap-2">
              {/* Show Admin Panel link for outlet admins and super admins */}
              {(user.role === 'outlet_admin' || user.role === 'super_admin') && (
                <Link
                  href="/admin/dashboard"
                  className="flex h-10 items-center justify-center rounded-full bg-blue-100 px-3 hover:bg-blue-200 transition-colors"
                  title="Admin Panel"
                >
                  <Store size={18} className="text-blue-700 mr-1" />
