...
const seedDatabase = async () => {
  try {
    ...
    // Create Outlet Admins and Outlets
    const outlets = [
      {
    ...
    // Create sample menu items for each outlet
    const menuItems = {
      'Hotspot (Nescafe)': [
    ...
    for (const outlet of createdOutlets) {
      ...
      for (const itemData of items) {
        const menuItem = new MenuItem({
          ...
          image: `/images/menu/${itemData.name.toLowerCase().replace(/\s+/g, '-')}.jpg`,
      ...
      console.log(`Added menu items for: ${outlet.name}`);
    }
